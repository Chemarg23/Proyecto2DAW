...
const validationSchema = Yup.object().shape({
  ...
  video: Yup.mixed()
    ...
    .test("fileType", "Solo se permiten archivos de tipo MP4", (value) => {
...
const [loader, setLoader] = useState(false)
  const addEpisode = async (values, setError) => {
    ...
    episodeService
      .add(values, serie.id)
      .then((response) => {
        setEpisodes((prev) => [...prev, response]);
        setLoader(false)
        setState(false);
      })
      .catch((err) =>{
          ...
          {({ setFieldValue, errors }) => (
            <Form>
              <div>
                ...
                > {loader  && <i className="fa-solid fa-spinner fa-spin me-3"></i>}
                  Añadir
                </Button>
              </div>
            </Form>
